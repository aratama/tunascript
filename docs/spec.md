...
`i32` は Wasm の `i32` に対応する32bit整数型です。主に `prelude` / `server` の低レベル `extern function` 宣言で使うための型で、通常のアプリケーションコードでは `i64` の使用を推奨します。
...
```typescript
type MyType = { name: string; age: i64 };
...
リテラル型はその値そのものしか代入できないため、`string` や `i64` などの汎用的な型からの代入はエラーになります。逆に、リテラル型はより広い型（`string` / `i64` / `boolean` / `f64`）には代入可能なので、タグ付きユニオンの `"type"` プロパティに使うと `switch` での絞り込みが強力になります。
...
- 異なる型の比較・暗黙変換は行いません。
- `i64` と `f64` の比較は **コンパイルエラー** になります。
- `i32` と `i64` / `f64` の暗黙変換は行いません。
...
// タプルでも使用可能
const tuple: [i64, string] = [1, "hello"]
...
```typescript
const obj: { name: string; age: i64 } = { name: "Alice", age: 30 };
const { name, age } = obj; // name="Alice", age=30

// 型注釈も可能（通常は不要です）
const { name: string, age: i64 } = obj;
```
...
- 宣言構文: `function add(a: i64, b: i64): i64 { return a + b }`
- 外部実装宣言: `extern function string_length(str: string): i64`
...
この例では `map` が `(value: i64) => U` を期待しているため、`value` は `i64` と推論され、戻り値も自動的に `i64` になります。`reduce` では累積値 `acc` の型として初期値 `0` の型 (`i64`) が知られているので、パラメータと戻り値の型がすべて推論されます。
...
`const obj: { func: (a: i64) => void }`であっても、その関数を`obj.func(a)`という構文で呼ぶことはできません。括弧を追加して`(obj.func)(a)`のように書く必要があります。
...
- 関数呼び出しの引数リストは末尾カンマを許容しません（例: `log("hello", )` はコンパイルエラー）。
...
- `+ - * / %`
- `i64` は整数演算、`f64` は浮動小数点演算です。
- `+` は **string + string のみ**連結になります。

### 5.2 比較

- `==` / `!=`
- `i64` / `f64` / `boolean` / `string` / `array` / `object` / `null` / `undefined` に対応します。
- 参照比較ではなく **値の比較** になります。
...
const a: i64 | undefined = if (v) { 42 }
const b: i64 | string = if (v) { 42 } else { "42" }
...
```typescript
function first(xs: i64[]): i64 | error {
  const value: i64 = xs[0]?
  return value
}
```
...
const message = switch (v) {
  case n as i64: "v is i64: " + to_string(n)
...
- `${expr}` に埋め込めるのは `string` / `i64` / `f64` / `boolean`（およびそれらのUnion）です。
...
- 配列/タプル/オブジェクト/型引数/分割代入などのカンマ区切りは末尾カンマを許容します。
- インデックス: `arr[i]`（`i` は `i64`）です。戻り値は `T | error`（要素型 `T` と `error` のUnion）になります。
