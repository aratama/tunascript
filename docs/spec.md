...
リテラル型はその値そのものしか代入できないため、`string` や `i64` などの汎用的な型からの代入はエラーになります。逆に、リテラル型はより広い型（`string` / `i64` / `boolean` / `number`）には代入可能なので、タグ付きユニオンの `"type"` プロパティに使うと `switch` での絞り込みが強力になります。
...
- 異なる型の比較・暗黙変換は行いません。
- `i64` と `number` の比較は **コンパイルエラー** になります。
- `i32` と `i64` / `number` の暗黙変換は行いません。
...
- `+ - * / %`
- `i64` は整数演算、`number` は浮動小数点演算です。
- `+` は **string + string のみ**連結になります。

### 5.2 比較

- `==` / `!=`
- `i64` / `number` / `boolean` / `string` / `array` / `object` / `null` / `undefined` に対応します。
- 参照比較ではなく **値の比較** になります。
...
- `${expr}` に埋め込めるのは `string` / `i64` / `number` / `boolean`（およびそれらのUnion）です。
